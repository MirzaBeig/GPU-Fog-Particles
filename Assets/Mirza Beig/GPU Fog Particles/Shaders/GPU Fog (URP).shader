	...
	SubShader
	{
        ...
        Pass
        {
			...
			half4 frag(VertexOutput IN ) : SV_TARGET
			{
				...
				float ParticleStableRandom43 = IN.ase_texcoord.z;
				float2 texCoord3 = IN.ase_texcoord.xy * float2( 1,1 ) + ( ( _SimpleNoiseAnimation * _TimeParameters.x ) + ( ParticleStableRandom43 * 10.0 ) );
				float simpleNoise1 = SimpleNoise( texCoord3*_SimpleNoiseScale );
				float SimpleNoise18 = saturate( (0.0 + (simpleNoise1 - _SimpleNoiseRemap) * (1.0 - 0.0) / (1.0 - _SimpleNoiseRemap)) );
				float lerpResult34 = lerp( 1.0 , SimpleNoise18 , _SimpleNoiseAmount);
				...
				float time2 = mulTime6;
				float2 voronoiSmoothId2 = 0;
				float2 texCoord7 = IN.ase_texcoord.xy * float2( 1,1 ) + ( (_VoronoiNoiseAnimation).xy * _TimeParameters.x );
				float2 coords2 = texCoord7 * _VoronoiScale;
				float2 id2 = 0;
				float2 uv2 = 0;
				float voroi2 = voronoi2( coords2, time2, id2, uv2, 0, voronoiSmoothId2 );
				...
				float cameraDepthFade87 = (( eyeDepth -_ProjectionParams.y - _CameraDepthFadeOffset ) / _CameraDepthFadeRange);
				float CameraDepthFade88 = saturate( cameraDepthFade87 );
				float2 texCoord76 = IN.ase_texcoord.xy * float2( 2,2 ) + float2( -1,-1 );
				float RadialMask79 = saturate( ( 1.0 - length( texCoord76 ) ) );
				float4 Albedo81 = ( _Albedo * IN.ase_color );
				
				surfaceDescription.Alpha = saturate( ( RemappedNoise60 * SurfaceDepthFade55 * CameraDepthFade88 * RadialMask79 * Albedo81.a ) );
        ...
        Pass
        {
			...
			{
				SurfaceDescription surfaceDescription = (SurfaceDescription)0;
				float ParticleStableRandom43 = IN.ase_texcoord1.z;
				float2 texCoord3 = IN.ase_texcoord1.xy * float2( 1,1 ) + ( ( _SimpleNoiseAnimation * _TimeParameters.x ) + ( ParticleStableRandom43 * 10.0 ) );
				float simpleNoise1 = SimpleNoise( texCoord3*_SimpleNoiseScale );
				float SimpleNoise18 = saturate( (0.0 + (simpleNoise1 - _SimpleNoiseRemap) * (1.0 - 0.0) / (1.0 - _SimpleNoiseRemap)) );
				float lerpResult34 = lerp( 1.0 , SimpleNoise18 , _SimpleNoiseAmount);
				...
				float time2 = mulTime6;
				float2 voronoiSmoothId2 = 0;
				float2 texCoord7 = IN.ase_texcoord1.xy * float2( 1,1 ) + ( (_VoronoiNoiseAnimation).xy * _TimeParameters.x );
				float2 coords2 = texCoord7 * _VoronoiScale;
				float2 id2 = 0;
				float2 uv2 = 0;
				float voroi2 = voronoi2( coords2, time2, id2, uv2, 0, voronoiSmoothId2 );
				...
				float cameraDepthFade87 = (( eyeDepth -_ProjectionParams.y - _CameraDepthFadeOffset ) / _CameraDepthFadeRange);
				float CameraDepthFade88 = saturate( cameraDepthFade87 );
				float2 texCoord76 = IN.ase_texcoord1.xy * float2( 2,2 ) + float2( -1,-1 );
				float RadialMask79 = saturate( ( 1.0 - length( texCoord76 ) ) );
				float4 Albedo81 = ( _Albedo * IN.ase_color );
...
/*ASEBEGIN
Version=18933
0;617.3334;1278;741.3333;1971.91;734.9522;1.468581;True;False
...
ASEEND*/
//CHKSM=F8F1EFEB0D986C98A331E86CF3B97F83B7CDB97A